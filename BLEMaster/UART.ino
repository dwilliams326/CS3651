...
int BLEsetup() {
  ...
  uint8_t bdaddr[] = {0x10, 0x34, 0x00, 0xE1, 0x80, 0x02};
  ...
  if (ret == BLE_STATUS_SUCCESS) {
    ...
    PRINTF("Error  adding UART service.\n");
  }
...
void GAP_Central_General_Discovery_Procedure(void)
{
  ...
  */   
  //ret = aci_gap_start_general_conn_establish_proc_IDB05A1(PASSIVE_SCAN, 0x4000, 0x4000, 0x00, 0x00);
...
void setConnectable(void)
{
  ...
  PRINTF("GAP_Central_General_Discovery_Procedure.\n");
...
void HCI_Event_CB(void *pckt)
{
  ...
  switch (event_pckt->evt) {
    ...
    case EVT_DISCONN_COMPLETE:
      {
        PRINTF("Fired EVT_DISCONN_COMPLETE event.\n");
    ...
    case EVT_LE_META_EVENT:
      {
        ...
        switch (evt->subevent) {
          case EVT_LE_CONN_COMPLETE:
            {
              PRINTF("Fired EVT_LE_CONN_COMPLETE event.\n");
           ...
           case EVT_LE_ADVERTISING_REPORT:
            {
              PRINTF("Fired EVT_LE_ADVERTISING_REPORT event.\n");
              le_advertising_info *pr = (void *)(evt->data+1);
                            
              tBleStatus ret;
              if (pr->evt_type == 0) {
                PRINTF("This is bdaddr %d\n", pr->bdaddr);
                ret = aci_gap_create_connection(0x4000, 0x4000, pr->bdaddr_type, pr->bdaddr, 0, 40, 40, 0, 60, 2000, 2000);
                if (ret != BLE_STATUS_SUCCESS) {
                  PRINTF("Failed aci_gap_create_connection  %x\n", (uint8_t)ret);
                } 
              }
             }
            break; 
    ...
    case EVT_VENDOR:
      {
        ...
        switch (blue_evt->ecode) {
          ...
          {
            PRINTF("Fired EVT_BLUE_GAP_PROCEDURE_COMPLETE event.\n");
          }
          break;
        }
      }
